...
const source2016 = {
  ...
  'blue': '82, 158, 204',
  'purple': '167, 125, 194',
  /* 'pink': '116, 128, 137', */ 'pink': '219, 117, 187',
  'accent': '82, 158, 204',
  'secondary-accent': '229, 231, 234',
  'follow': '243, 248, 251',
...
const source2013 = {
  ...
  'blue': '99, 167, 209',
  'purple': '176, 138, 200',
  /* 'pink': '130, 141, 149', */ 'pink': '223, 121, 191',
  'accent': '99, 167, 209',
...
const sourceDecision2016 = {
  ...
  'blue': '99, 166, 208',
  'purple': '176, 138, 199',
  /* 'pink': '130, 141, 148', */ 'pink': '223, 121, 191',
  'accent': '247, 37, 0',
  'secondary-accent': '245, 245, 245',
  'follow': '243, 248, 251',
...
const generate = (source, name) => {
  ...
  [
    ...
    [3, 5, 10, 15, 20, 30, 40, 50, 60, 70, 80, 85, 90, 95, 100].forEach((num) => {
      if (colorName === 'Navy') {
        const result = toWhite((100 - num) / 100).toString({ format: 'rgba_number' });
        designTokensMut[`${key}${num}`] = result;
      } else {
        const result = (
          num > 50 ? toBlack((num * 2 - 100) / 100) : toWhite((100 - num * 2) / 100)
        ).toString({ format: 'rgba_number' });
        designTokensMut[`${key}${num}`] = result;
      }
    });
  ...
  [
    ...
    if (Array.isArray(color)) {
      ...
      // designTokensMut[`colorRainbow${i}`] = range(0.5).toString({ format: 'rgba_number' });
    } else {
  ...
  console.log(`${name} designTokens`, designTokens);

  const palette = trueBlueTokensToPalette(designTokens);
  console.log(`${name} palette`, palette);
